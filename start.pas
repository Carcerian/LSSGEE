...
uses
  ...
  LResources,
  {shellAPI, wininet,}
  LCLType, {InterfaceBase,} LazFileUtils, Process, ActnList, BioFiles;
...
var
  Registry: TRegistry;
  DarkModeValue: integer;
begin
  Result := false;
  Registry := TRegistry.Create();
  try
    Registry.RootKey := HKEY_CURRENT_USER;
    if Registry.OpenKey('Software\Microsoft\Windows\CurrentVersion\Themes\Personalize', false) then
    begin
      try
        if Registry.ValueExists('AppsUseLightTheme') then
        begin
          DarkModeValue := Registry.ReadInteger('AppsUseLightTheme');
          Result := (DarkModeValue = 0); // 0 = dark mode ON, 1 = light mode ON
        end;
      except
        Result := false; // Default to light mode if error
      end;
      Registry.CloseKey();
    end;
  finally
...
// Recursively swap colors in a component and all its children
procedure SwapComponentColors(Component: TComponent);
var
  i: integer;
  Control: TControl;
begin
  ...
  if Component.ComponentCount > 0 then
    for i := 0 to Component.ComponentCount - 1 do
      SwapComponentColors(Component.Components[i]);
end;


// Apply light mode color scheme to all forms in the application
procedure ApplyLightModeColors;
var
  i: integer;
  Form: TCustomForm;
begin
  // Iterate through all open forms and swap their colors
  for i := 0 to Screen.FormCount - 1 do
  begin
    Form := Screen.Forms[i];
    if Form <> nil then
      SwapComponentColors(Form);
  end;
end;
...
begin
    ...
    // Auto-detect Windows dark mode and apply appropriate colors
    // Dark mode = lilac theme (already in .lfm files)
    // Light mode = swap to brown/cream theme for better readability
